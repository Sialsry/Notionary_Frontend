...
const PostForm = () => {
  ...
  const isFormValid =
    ...
    (
      !isWorkspaceShared || 
      (fk_workspace_id && selectedPageId && (Array.isArray(selectedPageId) ? selectedPageId.length > 0 : true))
    );
const handleSubmit = (e) => {
  ...
  if (isWorkspaceShared) {
    if (!fk_workspace_id || !selectedPageId || (Array.isArray(selectedPageId) && selectedPageId.length === 0)) {
      alert('워크스페이스에 게시글을 공유하려면 워크스페이스 및 페이지를 선택하세요.');
      return;
    }
  }
  ...
  if (isWorkspaceShared) {
    ...
    formData.append('selectedPageIds', Array.isArray(selectedPageId) ? selectedPageId.join(',') : selectedPageId);
  } else {
